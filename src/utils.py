...
def get_regression_metrics(y_predict_test, y_test, y_predict_train,
                           y_train):
    """
    ...
    metrics_diff = list(map(lambda x: x[1] - x[0], zip(metrics_train,
                                                       metrics_test)))
...
def get_classifier_metrics(y_predict_test, y_test, y_predict_train,
    ...
    return pd.DataFrame(data=[metrics_train, metrics_test],
                        columns=['Accuracy', 'F1 Score', 'Precision', 'Recall'], index=['Train set', 'Test set'])